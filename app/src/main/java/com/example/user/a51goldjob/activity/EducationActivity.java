...
import com.example.user.a51goldjob.bean.Education;
...
public class EducationActivity extends BaseActivity {
	...
	protected void actionPerformed_itemOnClick(View v) {
		...
		if (viewId == R.id.education_list_item_delete_imageview) {
			Object tag = v.getTag();
			final int position = Integer.parseInt(tag.toString());
			final Education item = (Education) education_listview.getItemAtPosition(position);
			...
			JsonUtils.put(jsonObject, "educationid", item.getEducationid());
			JsonUtils.put(jsonObject, "activeflag", activeflag); 

			final String data = RequestData.getData(jsonObject, Transcodes.TC_EDUCATION_SET, false);
			Https.post(this, Transcodes.TC_EDUCATION_SET, data, new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true){
	...
	protected void actionPerformed_listItemOnClick(AdapterView<?> parent,
		...
		// TODO Auto-generated method stub
		final Education item = (Education) education_listview.getItemAtPosition(position);
		Class<?> to = EducationEditActivity.class;
		ActivityUtils.startActivityForResult(handler,
				EducationActivity.this, to, new IIntentHandler() {
					public void putExtra(Intent intent) {
						String resumeid = getIntent().getExtras()
								.getString("resumeid");
						intent.putExtra("resumeid", resumeid);
						intent.putExtra("id", item.getEducationid());
					}
				});
	}
	...
	private void showDelete(int visiable) {
		int childCount = education_listview.getChildCount();
		View view = null;
		ImageView imageView = null;
		for (int i = 0; i < childCount; i++) {
			view = education_listview.getChildAt(i);
			imageView = (ImageView) view
					.findViewById(R.id.education_list_item_delete_imageview);
			if (imageView == null) {
				continue;
			}
			imageView.setTag(i);
	...
	private void init() {
		// TODO Auto-generated method stub
		// 添加按钮
		Button addBtn = (Button) findViewById(R.id.add_education);// 获取按钮资源
	...
	protected void actionPerformed_onItemClick(AdapterView<?> parent,
			View view, int position, long id) {
		final Education item = (Education) parent
	...
	protected List<?> parseDataList(ResponseData responseData) throws Exception {
		...
		List<Education> datas = gson.fromJson(data,
		...
		Https.post(this, Transcodes.TC_EDUCATION, data,
				new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true) {
					protected void _onSuccess(ResponseInfo<String> response)
						...
						education_listview.setOnItemClickListener(listItemOnClickListener);
					}
				});
	}
	...
	protected BaseAdapter createListViewAdapter() {
		...
		BeanAdapter adapter = new BeanAdapter(this, dataList,
				R.layout.activity_education_list_item, new String[] {
						"school", "starttime", "endtime","educationid" }, new int[] {
						R.id.activity_education_list_item_school,
						R.id.activity_education_list_item_starttime,
						R.id.activity_education_list_item_endtime,
						R.id.activity_education_list_item_educationid});
		setAdapter(adapter);
		return adapter;
	}
