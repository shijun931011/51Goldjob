...
import com.example.user.a51goldjob.bean.Cert;
...
@ContentView(R.layout.activity_cert)
public class CertActivity extends BaseActivity {
	...
	protected void actionPerformed_itemOnClick(View v) {
		...
		final Cert item = (Cert) cert_listview.getItemAtPosition(position);
		...
		JsonUtils.put(jsonObject, "certid", item.getCertid());
		JsonUtils.put(jsonObject, "activeflag", activeflag); 
		if (viewId == R.id.cert_list_item_delete_imageview) {

			final String data = RequestData.getData(jsonObject, Transcodes.TC_CERT_SET, false);
			Https.post(this, Transcodes.TC_CERT_SET, data, new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true){
	...
	protected void actionPerformed_listItemOnClick(AdapterView<?> parent,
		...
		// TODO Auto-generated method stub
		final Cert item = (Cert) cert_listview.getItemAtPosition(position);
		Class<?> to = CertEditActivity.class;
		ActivityUtils.startActivityForResult(handler,
				CertActivity.this, to, new IIntentHandler() {
					public void putExtra(Intent intent) {
						String resumeid = getIntent().getExtras()
								.getString("resumeid");
						intent.putExtra("resumeid", resumeid);
						intent.putExtra("id", item.getCertid());
					}
				});
	}
	...
	private void showDelete(int visiable) {
		int childCount = cert_listview.getChildCount();
		View view = null;
		ImageView imageView = null;
		for (int i = 0; i < childCount; i++) {
			view = cert_listview.getChildAt(i);
			imageView = (ImageView) view
					.findViewById(R.id.cert_list_item_delete_imageview);
			if (imageView == null) {
				continue;
			}
			imageView.setTag(i);
	...
	private void init() {
		// TODO Auto-generated method stub
		// 添加按钮
		Button addBtn = (Button) findViewById(R.id.add_cert);// 获取按钮资源
	...
	protected List<?> parseDataList(ResponseData responseData) throws Exception {
		...
		List<Cert> datas = gson.fromJson(data,
				new TypeToken<List<Cert>>() {
				}.getType());
		return datas;
	}
		...
		Https.post(this, Transcodes.TC_CERT, data,
				new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true) {
					protected void _onSuccess(ResponseInfo<String> response)
						...
						Cert cert1=(Cert)list.get(0);
						String str1=cert1.getScore();
						Cert cert2=(Cert)list.get(1);
						String str2=cert2.getScore();
						dataList.addAll(list);
						// 实现列表的显示
						ListAdapter baseAdapter=createListViewAdapter();
						cert_listview
								.setAdapter(baseAdapter);
						//cert_listview.notify();
						cert_listview.setOnItemClickListener(listItemOnClickListener);
					}
				});
	}
	...
	protected BaseAdapter createListViewAdapter() {
		...
		BeanAdapter adapter = new BeanAdapter(this, dataList,
				R.layout.activity_cert_list_item, new String[] {
						"certname","score" }, new int[] {
						R.id.activity_cert_list_item_certname,
						R.id.activity_cert_list_item_certid});
		setAdapter(adapter);
		return adapter;
	}
