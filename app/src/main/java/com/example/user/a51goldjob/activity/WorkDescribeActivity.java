...
public class WorkDescribeActivity extends BaseFormActivity implements TextWatcher,OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
	    ...
	    orignStr=getIntent().getExtras().getString("WorkDesc");
	    if(orignStr==null){
	    	orignStr="";
	    }
...
//	   this.findViewById(id);
   ...
   if(v.getId()==R.id.title_left||v.getId()==R.id.topbar_return_btn){
	   if(relativeLayout.isClickable()){ //可点击则为保存
			confirmMessage(WorkDescribeActivity.this, true, "返回当前页面会丢失编辑内容是否要保存?");
		}else{
			this.finish();
		}
