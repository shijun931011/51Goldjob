...
import com.example.user.a51goldjob.bean.Training;
...
@ContentView(R.layout.activity_training)
public class TrainingActivity extends BaseActivity {
	...
	@ViewInject(R.id.training_listview)
	ListView training_listview;
	...
	protected void actionPerformed_itemOnClick(View v) {
		...
		final Training item = (Training) training_listview.getItemAtPosition(position);
		...
		JsonUtils.put(jsonObject, "trainid", item.getTrainid());
		JsonUtils.put(jsonObject, "activeflag", activeflag); 
		if (viewId == R.id.work_history_list_item_delete_imageview) {

			final String data = RequestData.getData(jsonObject, Transcodes.TC_TRAINING_SET, false);
			Https.post(this, Transcodes.TC_TRAINING_SET, data, new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true){
	...
	protected void actionPerformed_listItemOnClick(AdapterView<?> parent,
		...
		// TODO Auto-generated method stub
		final Training item = (Training) training_listview.getItemAtPosition(position);
		Class<?> to = TrainingEditActivity.class;
		ActivityUtils.startActivityForResult(handler,
				TrainingActivity.this, to, new IIntentHandler() {
	...
	private void showDelete(int visiable) {
		int childCount = training_listview.getChildCount();
		View view = null;
		ImageView imageView = null;
		for (int i = 0; i < childCount; i++) {
			view = training_listview.getChildAt(i);
			imageView = (ImageView) view
					.findViewById(R.id.training_list_item_delete_imageview);
			if (imageView == null) {
				continue;
			}
			imageView.setTag(i);
	...
	private void init() {
		// TODO Auto-generated method stub
		// 添加按钮
		Button addBtn = (Button) findViewById(R.id.add_training);// 获取按钮资源
	...
	protected List<?> parseDataList(ResponseData responseData) throws Exception {
		...
		List<Training> datas = gson.fromJson(data,
		...
		Https.post(this, Transcodes.TC_TRAINING, data,
				new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true) {
					protected void _onSuccess(ResponseInfo<String> response)
						...
						training_listview.setOnItemClickListener(listItemOnClickListener);
					}
				});
	}
	...
	protected BaseAdapter createListViewAdapter() {
		...
		BeanAdapter adapter = new BeanAdapter(this, dataList,
				R.layout.activity_training_list_item, new String[] {
						"subject", "starttime", "endtime","trainid" }, new int[] {
						R.id.activity_training_list_item_subject,
						R.id.activity_training_list_item_starttime,
						R.id.activity_training_list_item_endtime,
						R.id.activity_training_list_item_trainid});
		setAdapter(adapter);
		return adapter;
	}
