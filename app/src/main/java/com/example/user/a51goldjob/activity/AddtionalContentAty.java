...
public class AddtionalContentAty extends BaseFormActivity implements OnClickListener,TextWatcher{
...
//	   this.findViewById(id);
   ...
   if(v.getId()==R.id.title_left||v.getId()==R.id.topbar_return_btn){
	   if(relativeLayout.isClickable()){ //可点击则为保存
			confirmMessage(AddtionalContentAty.this, true, "返回当前页面会丢失编辑内容是否要保存?");
			this.findViewById(R.id.action_context_bar);
		}else{
			this.finish();
		}
