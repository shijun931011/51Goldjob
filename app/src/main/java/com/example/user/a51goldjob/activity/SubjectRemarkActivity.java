...
public class SubjectRemarkActivity extends BaseFormActivity implements TextWatcher,OnClickListener{
...
//	   this.findViewById(id);
   ...
   if(v.getId()==R.id.title_left||v.getId()==R.id.topbar_return_btn){
	   if(relativeLayout.isClickable()){ //可点击则为保存
			confirmMessage(SubjectRemarkActivity.this, true, "返回当前页面会丢失编辑内容是否要保存?");
		}else{
			this.finish();
		}
