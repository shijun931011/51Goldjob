...
import com.example.user.a51goldjob.bean.WorkHistory;
...
@ContentView(R.layout.activity_work_history)
public class WorkHistoryActivity extends BaseActivity {
	...
	@ViewInject(R.id.work_history_listview)
	ListView work_history_listview;
	...
	protected void actionPerformed_listItemOnClick(AdapterView<?> parent,
		...
		// TODO Auto-generated method stub
		final WorkHistory item = (WorkHistory) work_history_listview.getItemAtPosition(position);
		final String workid = item.getWorkid();
		Class<?> to = WorkHistoryEditActivity.class;
		ActivityUtils.startActivityForResult(handler,
				WorkHistoryActivity.this, to, new IIntentHandler() {
	...
	protected void actionPerformed_itemOnClick(View v) {
		...
		if (viewId == R.id.work_history_list_item_delete_imageview) {
			Object tag = v.getTag();
			...
			JsonUtils.put(jsonObject, "workid", item.getWorkid());
			JsonUtils.put(jsonObject, "activeflag", activeflag); 

			final String data = RequestData.getData(jsonObject, Transcodes.TC_WORK_HISTORY_SET, false);
			Https.post(this, Transcodes.TC_WORK_HISTORY_SET, data, new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true){
	...
	private void showDelete(int visiable) {
		int childCount = work_history_listview.getChildCount();
		View view = null;
		ImageView imageView = null;
		for (int i = 0; i < childCount; i++) {
			view = work_history_listview.getChildAt(i);
			imageView = (ImageView) view
					.findViewById(R.id.work_history_list_item_delete_imageview);
			if (imageView == null) {
				continue;
			}
			imageView.setTag(i);
	...
	private void init() {
		// TODO Auto-generated method stub
		// 添加按钮
		Button addBtn = (Button) findViewById(R.id.add_work_history);// 获取按钮资源
	...
	protected void actionPerformed_onItemClick(AdapterView<?> parent,
			View view, int position, long id) {
		final WorkHistory item = (WorkHistory) parent
		...
		ActivityUtils.startActivity(handler, this, to, new IIntentHandler() {
			public void putExtra(Intent intent) {
				String resumeid = getIntent().getExtras().getString("resumeid");
				intent.putExtra("resumeid", resumeid);
				intent.putExtra("workid", String.valueOf(item.getWorkid()));
			}
		});
	}
	...
	protected List<?> parseDataList(ResponseData responseData) throws Exception {
		...
		List<WorkHistory> datas = gson.fromJson(data,
				new TypeToken<List<WorkHistory>>() {
				}.getType());
		return datas;
	}
		...
		Https.post(this, Transcodes.TC_WORK_HISTORY, data,
				new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true) {
					protected void _onSuccess(ResponseInfo<String> response)
						...
						// 实现列表的显示
						work_history_listview
								.setAdapter(createListViewAdapter());
						work_history_listview.setOnItemClickListener(listItemOnClickListener);
					}
				});
	}
	...
	protected BaseAdapter createListViewAdapter() {
		...
		BeanAdapter adapter = new BeanAdapter(this, dataList,
				R.layout.activity_work_history_list_item, new String[] {
						"corpname", "starttime", "endtime" }, new int[] {
						R.id.activity_work_history_list_item_corpname,
						R.id.activity_work_history_list_item_starttime,
						R.id.activity_work_history_list_item_endtime }){
		protected void bindView(int position, View view) {
			super.bindView(position, view);
			
			ImageView imageView = (ImageView) view.findViewById(R.id.work_history_list_item_delete_imageview);
