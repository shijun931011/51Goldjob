...
import com.example.user.a51goldjob.bean.ProjectExperience;
...
@ContentView(R.layout.activity_project_experience)
public class ProjectExperienceActivity extends BaseActivity {
	...
	@ViewInject(R.id.project_experience_listview)
	ListView project_experience_listview;
	...
	protected void actionPerformed_itemOnClick(View v) {
		...
		final ProjectExperience item = (ProjectExperience) project_experience_listview.getItemAtPosition(position);
		...
		JsonUtils.put(jsonObject, "projectid", item.getProjectid());
		JsonUtils.put(jsonObject, "activeflag", activeflag); 
		if (viewId == R.id.project_experience_list_item_delete_imageview) {

			final String data = RequestData.getData(jsonObject, Transcodes.TC_PROJECT_EXPERIENCE_SET, false);
			Https.post(this, Transcodes.TC_PROJECT_EXPERIENCE_SET, data, new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true){
	...
	protected void actionPerformed_listItemOnClick(AdapterView<?> parent,
		...
		// TODO Auto-generated method stub
		final ProjectExperience item = (ProjectExperience) project_experience_listview.getItemAtPosition(position);
		Class<?> to = ProjectExperienceEditActivity.class;
		ActivityUtils.startActivityForResult(handler,
				ProjectExperienceActivity.this, to, new IIntentHandler() {
	...
	private void showDelete(int visiable) {
		int childCount = project_experience_listview.getChildCount();
		View view = null;
		ImageView imageView = null;
		for (int i = 0; i < childCount; i++) {
			view = project_experience_listview.getChildAt(i);
			imageView = (ImageView) view
					.findViewById(R.id.project_experience_list_item_delete_imageview);
			if (imageView == null) {
				continue;
			}
			imageView.setTag(i);
	...
	private void init() {
		// TODO Auto-generated method stub
		// 添加按钮
		Button addBtn = (Button) findViewById(R.id.add_project_experience);// 获取按钮资源
	...
	protected List<?> parseDataList(ResponseData responseData) throws Exception {
		...
		List<ProjectExperience> datas = gson.fromJson(data,
				new TypeToken<List<ProjectExperience>>() {
				}.getType());
		return datas;
	}
		...
		Https.post(this, Transcodes.TC_PROJECT_EXPERIENCE, data,
				new RequestCallBackAdapters.InfoRequestCallBackAdapter<String>(this, true) {
					protected void _onSuccess(ResponseInfo<String> response)
						...
						// 实现列表的显示
						project_experience_listview
								.setAdapter(createListViewAdapter());
						project_experience_listview.setOnItemClickListener(listItemOnClickListener);
					}
				});
	}
	...
	protected BaseAdapter createListViewAdapter() {
		...
		BeanAdapter adapter = new BeanAdapter(this, dataList,
				R.layout.activity_project_experience_list_item, new String[] {
						"projectname", "starttime", "endtime","projectid" }, new int[] {
						R.id.activity_project_experience_list_item_projectname,
						R.id.activity_project_experience_list_item_starttime,
						R.id.activity_project_experience_list_item_endtime,
						R.id.activity_project_experience_list_item_projectid});
		setAdapter(adapter);
		return adapter;
	}
